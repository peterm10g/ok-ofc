...
-Dprovider.dubbo.port=${provider.dubbo.port} \
-Dprovider.rest.port=${provider.rest.port} \
-Dprovider.ws.port=${provider.ws.port} \
-Dsnowflake.node=1"
...
echo "start ...."
java $JAVA_OPTS -cp $CLASS_PATH com.lsh.ofc.provider.bootstrap.ProviderMain ${APP} >> ${PHOME}/logs/out.log 2>&1 &
